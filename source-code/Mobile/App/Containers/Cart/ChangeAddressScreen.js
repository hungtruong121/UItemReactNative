...
import Style from './ChangeAddressScreenStyle';
...
class ChangeAddressScreen extends Component {
  ...
  static getDerivedStateFromProps(nextProps, prevState){
    ...
    if (address && address.length > 0) {
      const addressDefault = address.filter(item => item.isDefault === 1);
      if (addressDefault.length > 0) data.addressDefault = addressDefault[0]
    }
    return data;
  }
  ...
  handleSetDefaultAddress = data => {
    const { userId, language, userActions, cartActions } = this.props;
    ...
    try {
      userService.updateAddress([data], language).then(response => {
        if (response.success) {
          ...
          cartActions.updateAddress(data, userId);
        } else {
    ...
    return (
      <TouchableOpacity onPress={() => navigation.navigate(Screens.ADD_EDIT_ADDRESS, { isAdd: false, address: item })}>
        <Block style={Style.container} center row>
          <Block style={{width: '90%'}} flex={false}>
            <Text h3>
  ...
  render() {
    ...
    const { address, isOpen, addressDefault } = this.state;
    return (
...
ChangeAddressScreen.defaultProps = {};



ChangeAddressScreen.propTypes = {
