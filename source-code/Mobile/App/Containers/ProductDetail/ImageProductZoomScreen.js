import React, { Component } from "react";
import { connect } from "react-redux";
import { bindActionCreators } from 'redux';
import PropTypes from 'prop-types';
import Toast, { DURATION } from 'react-native-easy-toast';
import {
  Image, TouchableOpacity,
  FlatList, Dimensions,
  ScrollView
} from 'react-native';
import Carousel from 'react-native-snap-carousel';
import {
  Button, Block,Text, BaseModal, Cart,
  Card, Header, Input, Picker, TextCurrency
} from "../../Components";
import Icon from 'react-native-vector-icons/FontAwesome';
import { ProgressBar } from 'react-native-paper';
import { Badge } from 'react-native-paper';
import { strings } from '../../Locate/I18n';
import Style from './ImageProductZoomScreenStyle';
import ImageViewer from 'react-native-image-zoom-viewer';
import { Sizes, Colors, ApplicationStyles, Images } from '../../Theme';
import { resetUser } from '../../Utils/storage.helper';
import { Config } from '../../Config/index';
import { Screens } from '../../Utils/screens';
import { Constants } from '../../Utils/constants';
import { chunk } from '../../Utils/commonFunction';

const { width, height } = Dimensions.get('window');
const widthCarousel = width - (ApplicationStyles.marginHorizontal.marginHorizontal * 2);

class ImageProductZoomScreen extends Component {
  constructor(props) {
    super(props)
    this.state = {
      imageZoom: [],
      indexActive: 0,
      isEditing: false,
    }
  }

  static getDerivedStateFromProps(nextProps, prevState){
    const { navigation } = nextProps;
    const { isEditing } = prevState;
    let { imageZoom, index } = navigation.state.params;
    let data = {};
    if (!isEditing) {
      data.indexActive = index;
      if (imageZoom && imageZoom.length > 0) {
        imageZoom = imageZoom.map(item => {
          const imageUrl = `${Config.IMAGE_URL}${item}`;
          return {
            url: imageUrl
          }
        });
        data.imageZoom = imageZoom;
      }
    }
    
    return data;
  };

  componentDidMount = () => {
  }

  handleChange = index => {
    this.flatListImage.scrollToIndex({ animated: true, index});
    this.setState({
      isEditing: true,
      indexActive: index,
    });
  };

  handleClickImageChild = indexActive => {
    this.setState({
      isEditing: true,
      indexActive,
    });
  }

  renderItem = (item, index) => {
    const { indexActive } = this.state;
    return (
      <TouchableOpacity style={indexActive === index ? { borderColor: Colors.green, borderWidth: 1} : null} onPress={() => this.handleClickImageChild(index)}>
        <Image 
          source={{ uri: item.url }}
          style={Style.imageChild}
        />
      </TouchableOpacity>
    );
  };

  render() {
    // const { loading, userActions, navigation } = this.props;
    // // const { listOrder, refreshing, errorCode, isOpen } = this.state;
    // if ( errorCode === '401') {
    //   this.handleVisibleModal(false);
    //   resetUser();
    //   userActions.resetUser();
    //   navigation.navigate(Screens.WELCOME);
    // }
    const { navigation } = this.props;
    const { imageZoom, indexActive } = this.state;
    return (
      <Block style={Style.view}>
        <TouchableOpacity style={Style.back} onPress={() => navigation.goBack()}>
          <Text white><Icon name="close" size={25} /></Text>
        </TouchableOpacity>
        <ImageViewer
          index={indexActive}
          imageUrls={imageZoom}
          onChange={index => this.handleChange(index)}
        />
        <Block flex={false} style={{backgroundColor: Colors.black}}>
          <FlatList
            horizontal
            scrollEnabled
            showsHorizontalScrollIndicator={false}
            data={imageZoom}
            keyExtractor={(item, index) => `${index}`}
            renderItem={({item, index}) => this.renderItem(item, index)}
            ref={ref => this.flatListImage = ref }
          />
        </Block>
      </Block>
    );
  }
}

ImageProductZoomScreen.defaultProps = {};

ImageProductZoomScreen.propTypes = {};

const mapStateToProps = (state) => ({})

const mapDispatchToProps = (dispatch) => ({})

export default connect(
  mapStateToProps,
  mapDispatchToProps,
)(ImageProductZoomScreen);

